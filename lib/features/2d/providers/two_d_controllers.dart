...
import '../services/two_d_const.dart';

class TwoDNotifier extends StateNotifier<AsyncValue<List<TwoD>>> {
  TwoDNotifier(this.repository) : super(const AsyncLoading());
  final TwoDRepository repository;

  Future<void> getTwoDList(String section) async {
    final result = await repository.getTwoDList(section);
    state = result.fold(
  ...
  void reverse() {
    Set<String> list = {};
    List<TwoD> selectedNumber = state.asData?.value
    ...
    state = AsyncData([
      for (final s in (state.asData?.value ?? <TwoD>[]))
        if (list.contains(s.betNumber) && isTwoDAvalible(s))
    ...
    state.whenData((twoDList) {
      state = AsyncData([
        for (final td in twoDList)
          if (list.contains(td.betNumber) && isTwoDAvalible(td))
            td.copyWith(isSelected: true)
          else
            td
      ]);
